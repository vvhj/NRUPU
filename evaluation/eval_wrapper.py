...
def run_test_tusimple(net,data_root,work_dir,exp_name,griding_num, use_aux, distributed,batch_size = 8,istrain = False):
    ...
    for i,data in enumerate(dist_tqdm(loader)):
        ...
        for i,name in enumerate(names):
            ...
            fp.write(json_str+'\n')
    fp.close()
def run_test_qy(net,data_root,work_dir,exp_name,griding_num, use_aux, distributed,batch_size = 8,istrain = False):
    output_path = os.path.join(work_dir,exp_name+'.%d.txt'% get_rank())
    fp = open(output_path,'w')
    loader = get_test_loader(batch_size,data_root,'yq', distributed)
    for i,data in enumerate(dist_tqdm(loader)):
        imgs,names = data
        imgs = imgs.cuda()
        with torch.no_grad():
        ...
        for i,name in enumerate(names):
            tmp_dict = {}
            tmp_dict['lanes'] = generate_tusimple_lines(out[i],imgs[0,0].shape,griding_num)
            tmp_dict['h_samples'] = [17, 30, 42, 55, 67, 80, 92, 105, 117, 130, 142, 155, 167, 
            180, 192, 205, 217, 229, 242, 255, 267, 280, 292, 305, 317, 330, 342, 355, 367,
             380, 392, 405, 417, 430, 442, 455, 467, 480, 492, 504, 517, 530, 542, 555, 
             567, 580, 592, 605, 617, 630, 642, 655, 667, 680, 692, 705]
            tmp_dict['raw_file'] = name
            tmp_dict['run_time'] = 10
            json_str = json.dumps(tmp_dict)
...
def run_test_yqpd(net,data_root,work_dir,exp_name,griding_num, use_aux, distributed,batch_size = 8,istrain = False): 
    output_path = os.path.join(work_dir,exp_name+'.%d.txt'% get_rank())
    fp = open(output_path,'w')
    loader = get_test_loader(batch_size,data_root,'Yqpd', distributed)
    for i,data in enumerate(dist_tqdm(loader)):
        imgs,names = data
        imgs = imgs.cuda()
        with torch.no_grad():
...
def combine_yq_test(work_dir,exp_name):
    size = get_world_size()
    all_res = []
    
    output_path = os.path.join(work_dir,exp_name+'.%d.txt'% 0)
    with open(output_path, 'r') as fp:
        res = fp.readlines()
    #all_res.extend(res)
...
def eval_lane(net, dataset, data_root, work_dir, griding_num, use_aux, distributed,istrain=False):
    net.eval()
    if dataset == 'CULane':
        ...
        run_test_tusimple(net, data_root, work_dir, exp_name, griding_num, use_aux, distributed,istrain=istrain)
        synchronize()  # wait for all results
        if is_main_process():
        ...
        exp_name = 'yqpd_eval_tmp'
        run_test_qy(net, data_root, work_dir, exp_name, griding_num, use_aux, distributed,istrain=istrain)
        synchronize()  # wait for all results
        if is_main_process():
            combine_yq_test(work_dir,exp_name)
            res = LaneEval.bench_one_submit(os.path.join(work_dir,exp_name + '.txt'),os.path.join(data_root,'test_label.json'))
            res = json.loads(res)
            for r in res:
                dist_print(r['name'], r['value'])
