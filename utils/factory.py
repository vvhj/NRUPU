...
def get_loss_dict_fail(cfg):

    if cfg.use_aux:
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'aux_loss'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss(), torch.nn.CrossEntropyLoss()],
            'weight': [1.0, cfg.sim_loss_w, 1.0],
            'data_src': [('cls_out', 'cls_label'), ('cls_out',), ('seg_out', 'seg_label')]
        }
    else:
        loss_dict = {
            'name': ['cls_loss', 'relation_loss'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss()],
            'weight': [1.0, cfg.sim_loss_w],
...
def get_loss_dict(cfg):
    ...
    return loss_dict
def get_loss_dict_pd(cfg):

    if cfg.use_aux:
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'aux_loss', 'relation_dis','pd_loss'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss(), torch.nn.CrossEntropyLoss(), ParsingRelationDis()],
            'weight': [1.0, cfg.sim_loss_w, 1.0, cfg.shp_loss_w],
            'data_src': [('cls_out', 'cls_label'), ('cls_out',), ('seg_out', 'seg_label'), ('cls_out',)]
        }
        ...
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'relation_dis','pd_loss'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss(), ParsingRelationDis()],
            'weight': [1.0, cfg.sim_loss_w, cfg.shp_loss_w],
            'data_src': [('cls_out', 'cls_label'), ('cls_out',), ('cls_out',)]
        }
