...
def get_args():
    ...
    parser.add_argument('--config2', default = None, type = str)
    return parser
...
def save_model(net, optimizer, epoch,save_path, distributed,bncfg=None):
    if is_main_process():
        model_state_dict = net.state_dict()
        if bncfg==None:
            state = {'model': model_state_dict, 'optimizer': optimizer.state_dict()}
        else:
            state = {'model': model_state_dict, 'optimizer': optimizer.state_dict(),'bncfg':bncfg}
        # state = {'model': model_state_dict}
        assert os.path.exists(save_path)
        model_path = os.path.join(save_path, 'ep%03d.pth' % epoch)
        torch.save(state, model_path)
def save_model_best(net, optimizer, epoch,save_path, distributed,bncfg=None,stageid = None):
    if is_main_process():
        model_state_dict = net.state_dict()
        if bncfg==None:
            state = {'model': model_state_dict, 'optimizer': optimizer.state_dict()}
        else:
            state = {'model': model_state_dict, 'optimizer': optimizer.state_dict(),'bncfg':bncfg,'stageid':stageid}
        # state = {'model': model_state_dict}
        assert os.path.exists(save_path)
        model_path = os.path.join(save_path, 'best.pth')
        torch.save(state, model_path)
def save_new_model(net, optimizer, epoch,save_path, distributed,bncfg=None,stageid = None):
    if is_main_process():
        model_state_dict = net.state_dict()
        if bncfg==None:
            state = {'model': model_state_dict, 'optimizer': optimizer.state_dict()}
        ...
        torch.save(state, model_path)
def save_marge_model(net, save_path,bncfg=None):
    if is_main_process():
        model_state_dict = net.state_dict()
        if bncfg==None:
            state = {'model': model_state_dict}
        else:
            state = {'model': model_state_dict,'bncfg':bncfg}
        # state = {'model': model_state_dict}
        assert os.path.exists(save_path)
        model_path = os.path.join(save_path, 'bnmarge.pth')
        torch.save(state, model_path)
